...
namespace RenderHeads.Media.AVProVideo
{
	...
	public class MediaPlayer : MonoBehaviour
	{
		...
		public void EnableSubtitles(FileLocation fileLocation, string filePath)
		{
			if (m_Subtitles != null)
			{
				if (!string.IsNullOrEmpty(m_SubtitlePath))
				{
					string fullPath = GetPlatformFilePath(GetPlatform(), ref m_SubtitlePath, ref m_SubtitleLocation);
					...
					else
					{
						...
						try
						{
							...
							if (m_Subtitles.LoadSubtitlesSRT(System.IO.File.ReadAllText(fullPath)))
							{
								m_LoadSubtitles = true;
							}
						}
...
#region Support for Time Scale
		...
		private void UpdateEvents()
		{
			if (m_events != null && m_Control != null)
			{
				...
				{
					_previousSubtitleIndex = m_Subtitles.GetSubtitleIndex();
				}
			}
		}
			...
			if (m_events != null && m_Control != null && !hasFired)
			{
				switch (et)
				{
					...
					case MediaPlayerEvent.EventType.SubtitleChange:
						result = (_previousSubtitleIndex != m_Subtitles.GetSubtitleIndex());
						break;
				}
			}
			return result;
